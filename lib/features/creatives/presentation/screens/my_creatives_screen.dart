...
import '../../logic/purchased_creatives_notifier.dart';
import '../widgets/purchased_creative_card.dart';

class MyCreativesScreen extends ConsumerStatefulWidget {
  const MyCreativesScreen({super.key});

  @override
  ConsumerState<MyCreativesScreen> createState() => _MyCreativesScreenState();
}

class _MyCreativesScreenState extends ConsumerState<MyCreativesScreen> {
  ...
  void _onScroll() {
    if (_scrollController.position.pixels >=
      ...
      ref.read(purchasedCreativesProvider.notifier).loadMore();
    }
  }

  @override
  Widget build(BuildContext context) {
    final creativesState = ref.watch(purchasedCreativesProvider);
        ...
        data: (payload) {
          ...
          final filteredOrders = _searchQuery.isEmpty
              ...
              : payload.page.where((order) {
                  return order.creatives.any((creative) =>
                      creative.title
                          .toLowerCase()
              ...
              Expanded(
                child: filteredOrders.isEmpty
                    ...
                    : RefreshIndicator(
                        onRefresh: () async {
                          await ref
                              .read(purchasedCreativesProvider.notifier)
                              .refresh();
                        },
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return Column(
                              ...
                              children: [
                                if (index == 0 ||
                                    filteredOrders[index - 1].paymentRef !=
                                        order.paymentRef) ...[
                                  _buildOrderHeader(order),
                                  SizedBox(height: 12.h),
                                ],
                                ...order.creatives.map((creative) {
                                  // Filter individual creatives within order
                                  ...
                                  return Padding(
                                    ...
                                    child: PurchasedCreativeCard(
                                      creative: creative,
                                      orderStatus: order.status,
                                      purchaseDate: order.createdAt,
                                      onDownload: () =>
                                          _handleDownload(creative),
                                      onViewDetails: () =>
                                          _handleViewDetails(creative),
                                    ),
                                  );
                                }).toList(),
                              ],
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              Text(
                '${order.creatives.length} creative(s)',
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 8.h),
          Text(
            'Your purchased creatives will appear here',
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 8.h),
          Text(
            isNetworkError ? 'Please check your network connection' : error,
            style: AppTexts.bodyMedium(color: AppColors.grey600),
            textAlign: TextAlign.center,
          ),
          SizedBox(height: 8.h),
  ...
  Future<void> _handleDownload(creative) async {
    if (creative.fileUrl == null || creative.fileUrl.isEmpty) {
    ...
    try {
      final url = Uri.parse(creative.fileUrl);
      ...
      if (await canLaunchUrl(url)) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Opening ${creative.title}...'),
  ...
  void _handleViewDetails(creative) {
    context.push(
      '/creative-details/${creative.id}',
      extra: {
